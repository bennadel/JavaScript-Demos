import { DiagnosticCategory, JsxEmit, ModuleKind, ModuleResolutionKind, ScriptTarget, clone, createProgram, createSourceFile, extend, flattenDiagnosticMessageText, normalizePath, parseConfigFileTextToJson, preProcessFile, resolveModuleName } from 'typescript';
...
function hasError(diags) {
    return diags.some(function (diag) { return (diag.category === DiagnosticCategory.Error); });
}

var logger$2 = new Logger({ debug: false });
var HTML_MODULE = "__html_module__";
var CompilerHost = (function () {
    function CompilerHost(options, builder) {
        if (builder === void 0) { builder = false; }
        this._reportedFiles = [];
        this._options = options || {};
        this._options.module = this.getEnum(this._options.module, ModuleKind, ModuleKind.System);
        this._options.target = this.getEnum(this._options.target, ScriptTarget, ScriptTarget.ES5);
        this._options.jsx = this.getEnum(this._options.jsx, JsxEmit, JsxEmit.None);
        ...
        this._options.moduleResolution = ModuleResolutionKind.Classic;
        ...
        if ((this._options.target !== ScriptTarget.ES2015) && (this._options.module !== ModuleKind.ES2015))
        ...
        if (this._options.targetLib) {
            ...
            this.options.targetLib = this.getEnum(this._options.targetLib, ScriptTarget, ScriptTarget.ES2015);
        }
    ...
    CompilerHost.prototype.getDefaultLibFilePaths = function () {
        if (this._options.targetLib === ScriptTarget.ES5)
            return ['typescript/lib/lib.d.ts'];
        else if (this._options.targetLib === ScriptTarget.ES2015)
    ...
    CompilerHost.prototype.addFile = function (fileName, text) {
        ...
        if (!file) {
            this._files[fileName] = createSourceFile(fileName, text, this._options.target);
    ...
    CompilerHost.prototype.resolveModuleNames = function (moduleNames, containingFile) {
        ...
        return moduleNames.map(function (modName) {
            ...
            if (isHtml(modName) && _this._options.supportHtmlImports) {
                ...
                return resolveModuleName(modName, containingFile, _this._options, _this).resolvedModule;
            }
        });
    };
    return CompilerHost;
...
var Transpiler = (function () {
    function Transpiler(host) {
        this._host = host;
        this._options = clone(this._host.options);
    ...
    Transpiler.prototype.transpile = function (sourceName) {
        ...
        if (!file.output) {
            var program = createProgram([sourceName], this._options, this._host);
...
var Resolver = (function () {
    ...
    Resolver.prototype.resolve = function (sourceName) {
        ...
        if (!file.pendingDependencies) {
            var info = preProcessFile(file.text, true);
...
var TypeChecker = (function () {
    ...
    TypeChecker.prototype.getAllDiagnostics = function (candidates) {
        ...
        var program = createProgram(filelist, this._options, this._host);
...
function formatErrors(diags, logger) {
    diags.slice(0, 10)
        ...
        var messageText = flattenDiagnosticMessageText(diag.messageText, "\n");
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 2:
                    ...
                    result = parseConfigFileTextToJson(tsconfigAddress, tsconfigText);
                    if (result.error) {
                        formatErrors([result.error], logger$1);
                        throw new Error("failed to load tsconfig from " + tsconfigAddress);
                    }
                    files = result.config.files;
                    return [2 /*return*/, extend(extend({ tsconfigAddress: tsconfigAddress, files: files }, sjsconfig), result.config.compilerOptions)];
...
function translate(load) {
    ...
    return factory.then(function (_a) {
        ...
        if (isTypescriptDeclaration(load.address)) {
            if (loader.builder && (host.options.module == ModuleKind.ES2015)) {
                load.source = null;
            ...
            if (!host.options.autoDetectModule) {
                if (host.options.module === ModuleKind.System)
                    load.metadata.format = 'register';
                else if (host.options.module === ModuleKind.ES2015)
                    load.metadata.format = 'esm';
                else if (host.options.module === ModuleKind.CommonJS)
                    load.metadata.format = 'cjs';
            }
        }
        if (loader.builder)
...
function validateOptions(options) {
    if ((options.module != ModuleKind.System) && (options.module != ModuleKind.ES2015)) {
        logger.warn("transpiling to " + ModuleKind[options.module] + ", consider setting module: \"system\" in typescriptOptions to transpile directly to System.register format");
    }
}
